    ...
    return (
      ...
      >
        <div className="flex items-start justify-between gap-2">
          <div className="text-slate-300 opacity-90 [&>svg]:h-6 [&>svg]:w-6" aria-hidden>{icon}</div>
          {variation !== undefined && (
            <span
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard icon={<Hash />} label="Stock total (unidades)" value={formatGs(totalUnidades)} color="text-[#0EA5E9]" />
        ...
        <motion.div whileHover={{ y: -2 }} className="col-span-2 bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 p-6 transition-shadow hover:shadow-md">
          ...
          {criticos.length === 0 ? (
            <div className="flex items-center gap-2 text-[var(--badge-success-text)] bg-[var(--badge-success-bg)] rounded-lg px-4 py-3 text-sm">
              <span><CheckCircle2 className="inline h-4 w-4 align-[-0.125em]" aria-hidden /></span> Todos los productos tienen stock suficiente.
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-200">
                  {criticos.map(p => (
                    <tr key={p.id} className={`${p.stock_actual <= 0 ? "bg-red-50/40 dark:bg-red-900/10" : "bg-amber-50/30 dark:bg-amber-900/10"} hover:bg-opacity-80 transition-colors`}>
                      ...
                      <td className="px-3 py-2.5 text-xs text-slate-600 dark:text-slate-400 tabular-nums">{p.stock_minimo} {p.unidad_medida}</td>
                      <td className="px-3 py-2.5">
                      ...
                      <td className="px-3 py-2.5 text-xs text-slate-600 dark:text-slate-400">{proveedorMap[String(p.id)] ?? "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <tbody className="divide-y divide-slate-200">
                {topPorValor.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-3 py-2.5 text-xs font-medium text-slate-800 dark:text-slate-200">{p.nombre}</td>
                    <td className="px-3 py-2.5 font-mono text-xs text-slate-600 dark:text-slate-400">{p.sku}</td>
                    <td className="px-3 py-2.5 text-xs tabular-nums text-slate-700 dark:text-slate-300">{p.stock_actual}</td>
                    <td className="px-3 py-2.5 text-xs tabular-nums text-slate-600 dark:text-slate-400">Gs. {formatGs(p.costo_promedio)}</td>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard icon={<Ticket />} label="Ticket promedio"   value={`Gs. ${formatGsFull(ticketProm)}`}
          sub={`periodo: ${periodo}`} />
...
export default function DashboardPage() {
  ...
  // Instancia Cucina del Cuore: solo Ventas e Inventario.
  //
  ...
  const TAB_META: Record<TabDash, { label: string; icon: React.ReactNode }> = {
    comercial: { label: "Comercial", icon: <BarChart3 className="h-4 w-4" aria-hidden /> },
    financiero: { label: "Financiero", icon: <Wallet className="h-4 w-4" aria-hidden /> },
    inventario: { label: "Inventario", icon: <Package className="h-4 w-4" aria-hidden /> },
    ventas: { label: "Ventas", icon: <ShoppingCart className="h-4 w-4" aria-hidden /> },
  };
        ...
        >
          {effectiveTabs.map((tid) => {
            ...
            return (
              ...
              >
                <span className="inline-flex" aria-hidden>{meta.icon}</span>
                {meta.label}
              </button>
            );
          })}
